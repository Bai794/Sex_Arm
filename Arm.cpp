/*** 
 * @Author: HideMe
 * @Date: 2022-04-23 20:09:29
 * @LastEditTime: 2022-04-23 20:26:56
 * @LastEditors: your name
 * @Description: 
 * @FilePath: \Sex_Arm\Arm.cpp
 * @e-mail: 1269724114@qq.com
 */
//
// Created by bai on 2022/4/19.
//
...
Arm::Arm(float L_BS, float D_BS, float L_AM, float L_FA, float D_EW, float L_WT)
...
bool
Arm::SolveFK(const Arm::Joint6D_t &_inputJoint6D, Arm::Pose6D_t &_outputPose6D) {
    float q_in[6];
    float q[6];
    float cosq, sinq;
    float cosa, sina;
...
bool Arm::SolveIK(const Arm::Pose6D_t &_inputPose6D, const Joint6D_t &_lastJoint6D,
                  Arm::IKSolves_t &_outputSolves) {
    float qs[2];
    float qa[2][2];
    float qw[2][3];
    float cosqs, sinqs;
...
Arm::Joint6D_t operator-(const Arm::Joint6D_t &_joints1, const Arm::Joint6D_t &_joints2) {
    Arm::Joint6D_t tmp{};
    for (int i = 0; i < 6; i++)
